...
// TaskDriver Class 
//
// This is the TaskDriver class.
// It exercises a few simple examples of Task objects
...
public class TaskDriver {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 10; i++) {
			tasks[i] = new Task(i + 1, "Task with priority " + (i + 1));
		}
		...
		Task negativePriorityTask = new Task(-99, "Negative 99 priority task");
		...
		Task anotherTask = new Task(7,"Another Task with priority 7");
		comparisonResult = tasks[6].compareTo(anotherTask);
		System.out.println(prettyComparisonResult(comparisonResult));
		System.out.println();

		//using the interface type instead
		System.out.println("Using task 8 as Priority instead of Task");
		...
		System.out.println("Description (from cast): " + ((Task)interfaceReferenceTask).getDescription());
		
		System.out.println("Changing priority to 4:");
		interfaceReferenceTask.setPriority(4);
		//this still calls the Task toString since that's what the object actually is
		...
		Task randomTasks[] = new Task[6];
		
		Random rng = new Random();
		for (int i=0; i < 6; i++) {
			int randomValue = rng.nextInt(Priority.HIGHEST_PRIORITY) + 1;
			randomTasks[i] = new Task(randomValue, "#" + (i+1) + " - Random Priority: " + randomValue);
		}
